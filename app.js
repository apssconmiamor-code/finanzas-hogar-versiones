...
document.getElementById("btn-nuevo-movimiento").addEventListener("click", () => {
  ...
  // Filtros
  document.getElementById("filtro-caja").addEventListener("change", renderMovimientos);
...
function renderMovimientos() {
  const filtroC  = document.getElementById("filtro-caja").value;
  ...
  let filtrados = movimientos.filter(m => {
    if (filtroC && m.caja !== filtroC) return false;
    if (filtroT && m.categoria !== filtroT) return false;
    if (filtroK && m.concepto !== filtroK) return false;
...
filtrados.forEach(m => {
  ...
  if (caja && caja.moneda !== "COP") return;
  ...
  if (filtrados.length === 0) {
    list.innerHTML = `<div class="empty-state">
      <div class="empty-state-icon">📋</div>
      <div class="empty-state-text">No hay movimientos para mostrar.</div></div>`;
    return;
  }
  ...
  list.innerHTML = filtrados.map(m => {
    ...
    const tipoBadge = m.categoria
      ? `<span class="tipo-badge tipo-${m.categoria.toLowerCase().replace(" ","")}">${m.categoria}</span>`
      : "";
    const descHTML = m.descripcion
      ? `<div class="mov-descripcion">${m.descripcion}</div>` : "";
    return `<div class="movimiento-item">
      <div class="mov-icon ${cls}">${icono}</div>
      <div class="mov-info">
        <div class="mov-concepto">${m.concepto || "Sin concepto"} ${tipoBadge}</div>
        <div class="mov-meta">${m.fecha} · ${m.caja} · ${m.autor}</div>
        ${descHTML}
      </div>
      <div class="mov-right">
        <div class="mov-monto ${cls}">${signo}${formatMonto(Math.abs(m.monto))}</div>
...
function poblarFiltrosCajas() {
  const sel = document.getElementById("filtro-caja");
...
// =============================================
const TAB_TITLES = {
  cajas: "Inicio", movimientos: "Movimientos", proyeccion: "Proyección",
